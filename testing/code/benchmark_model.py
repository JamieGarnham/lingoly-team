...
def pipeline(
    ...
    if outfile == "modelname_questionsname.json":
        ...
        filename = (
            f"{model_name_str.split('/')[-1]}_lingoly{cust}{nc}{ct}{adv}{rp}{mp}.json"
        )
    else:
        filename = outfile
    ...
    if use_cache:
        if outfile == "modelname_questionsname.json":
            ...
            model_tmp_name = f"{model_tmp_name}_lingoly{cust}{nc}{ct}{adv}{rp}{mp}"
        else:
        ...
        if "_lingoly" in model_tmp_name:
            ...
            cached_dict = load_cache(f"{model_tmp_name}")
