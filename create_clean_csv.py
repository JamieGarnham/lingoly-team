#!/usr/bin/env python3
"""
Create a cleaner CSV version without newlines in context field.
"""
...
def create_clean_csv(original_jsonl_path, shuffled_jsonl_path, output_csv_path):
    """Create a clean CSV without newlines in fields."""
        ...
        if subprompts:
            for subprompt in subprompts:
                row = {
                    ...
                    'subprompt_question': clean_text(subprompt.get('question', '')),
                    'subprompt_answer': clean_text(str(subprompt.get('answer', ''))),
                    'preamble': clean_text(metadata.get('preamble', '')),
                    'context_preview': clean_text(context[:200]) + ('...' if len(context) > 200 else ''),
    ...
    fieldnames = [
        ...
        'subprompt_answer', 'preamble', 'context_preview', 'num_table_rows', 
        'was_shuffled', 'split_key'
    ]
...
def main():
    ...
    output_path = 'testing_17/data/splits/benchmark_single_obf_clean.csv'
    
    print(f"Creating clean CSV...")
    total_rows, shuffled_rows = create_clean_csv(original_path, shuffled_path, output_path)
    
    print(f"\nClean CSV created: {output_path}")
