#!/usr/bin/env python3
"""
Create CSV with one row per entry (173 entries + header = 174 total rows).
"""
...
def clean_text(text):
    """Clean text by removing newlines and limiting length."""
    cleaned = ' '.join(str(text).replace('\n', ' ').replace('\r', ' ').split())
    return cleaned[:500] + ('...' if len(cleaned) > 500 else '')


def create_entry_level_csv(original_jsonl_path, shuffled_jsonl_path, output_csv_path):
    """Create CSV with one row per entry."""
    ...
    for i, (orig_entry, shuf_entry) in enumerate(zip(original_entries, shuffled_entries)):
        ...
        # Get first subprompt as example
        ...
        row = {
            ...
            'main_prompt': clean_text(question_details.get('prompt', '')),
            'num_subprompts': num_subprompts,
            'first_subprompt_n': first_subprompt.get('questionpart_n', ''),
            'first_subprompt_question': clean_text(first_subprompt.get('question', '')),
            'first_subprompt_answer': clean_text(first_subprompt.get('answer', '')),
            'preamble': clean_text(metadata.get('preamble', '')),
            'context_preview': clean_text(context[:300]),
    ...
    fieldnames = [
        ...
        'preamble', 'context_preview', 'num_table_rows', 'was_shuffled', 'split_key'
    ]
...
def main():
    ...
    output_path = 'testing_17/data/splits/benchmark_single_obf_entries.tsv'
    
    print(f"Creating entry-level CSV...")
    total_entries, shuffled_entries = create_entry_level_csv(original_path, shuffled_path, output_path)
    
    print(f"\nEntry-level TSV created: {output_path}")
    ...
    print(f"Total rows in TSV: {total_entries + 1} (including header)")
