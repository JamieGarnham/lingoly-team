...
Processes all three formats (monolingual, pattern, rosetta) together and combines them into one CSV.
"""
...
def main():
    """
    Main function to process all chained prompt responses across all formats and create evaluation CSV with correctness.
    """
    parser = argparse.ArgumentParser(description='Evaluate chained prompt responses at the subquestion level with correctness across all formats')
    parser.add_argument('model_name', help='Model name (e.g., llama, gemini)')
    ...
    for format_type in formats:
        chained_dir = f"openrouter_runs/{args.model_name}_chained_{format_type}"
        print(f"Loading chained prompt responses from {chained_dir}...")
        
        try:
            responses_by_version = load_chained_prompt_responses(chained_dir, args.sample_size)
            print(f"Loaded responses from {len(responses_by_version)} versions for {format_type}: {list(responses_by_version.keys())}")
            
            print(f"Extracting subquestion data for {format_type}...")
            format_rows = extract_subquestion_data_from_chained(responses_by_version, format_mapping, format_fixed_mapping)
            all_rows.extend(format_rows)
    ...
    print("Saving to CSV...")
    # Determine model name from argument
    ...
    output_filename = f"chained_subquestion_eval_{model_name}_all_formats_{args.sample_size}.csv"
    output_path = analysis_dir / output_filename
    
    save_to_csv(all_rows, output_path)
    
    print(f"Done! Total rows: {len(all_rows)} (plus header)")
