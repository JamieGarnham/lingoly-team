#!/usr/bin/env python3
"""
Chained prompting approach for Pattern problems from benchmark_same_obf dataset.
...
def _read_jsonl_zip(path: str):
    """Read a password-protected jsonl.zip split (falls back to plain .jsonl)."""
...
def load_questions(file_path: str) -> Dict[int, List[Dict]]:
    ...
    for line in _read_jsonl_zip(file_path):
        if line.strip():
...
def create_question_prompt(question_data: Dict) -> str:
    ...
    question_text = f"Based on the patterns you have identified in the language, solve the following puzzle: \n{prompt}"
...
def load_cache(model_name: str, tmp_path: str = TMP_PATH) -> Dict:
    ...
    for root, dirs, files in os.walk(tmp_path):
        for file in files:
            if model_name + "_chained_pattern" in file and "tmp" in file:
...
def save_cache(responses: List[Dict], model_name: str, tmp_path: str = TMP_PATH):
    ...
    filename = f"{model_name}_chained_pattern_tmp_{timestamp}.json"
...
def main(
    benchmark_file: str = "../../data/splits/benchmark_same_obf_pattern.jsonl.zip",
    model_names: str = "DRY_RUN",
    max_problems: int = None,
    use_cache: bool = True
):
    """
    Run chained prompting evaluation on Pattern problems
    
    Args:
        benchmark_file: Path to the filtered Pattern benchmark file
    ...
    print(f"Loaded {len(questions_by_problem)} problems with Pattern format")
    ...
    for model_name in models_to_evaluate:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_file = Path(OUTFOLDER) / f"{model_name}_chained_pattern_evaluation_{timestamp}.jsonl"
