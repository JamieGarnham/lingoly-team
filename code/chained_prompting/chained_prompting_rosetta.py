#!/usr/bin/env python3
"""
Chained prompting approach for Rosetta problems from benchmark_same_obf dataset.
...
def load_cache(model_name: str, tmp_path: str = TMP_PATH) -> Dict:
    ...
    for root, dirs, files in os.walk(tmp_path):
        for file in files:
            if model_name + "_chained" in file and "tmp" in file:
...
def save_cache(responses: List[Dict], model_name: str, tmp_path: str = TMP_PATH):
    ...
    filename = f"{model_name}_chained_tmp_{timestamp}.json"
...
def main(
    benchmark_file: str = "../../data/splits/benchmark_same_obf_rosetta.jsonl.zip",
    model_names: str = "DRY_RUN",
    max_problems: int = None,
    use_cache: bool = True
):
    """
    Run chained prompting evaluation on Rosetta problems
    
    Args:
        benchmark_file: Path to the filtered Rosetta benchmark file
    ...
    print(f"Loaded {len(questions_by_problem)} problems with Rosetta format")
    ...
    for model_name in models_to_evaluate:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_file = Path(OUTFOLDER) / f"{model_name}_chained_evaluation_{timestamp}.jsonl"
