...
def load_format_mapping():
    ...
    with open('../../data/overall_question_format_mapping.csv', 'r') as f:
...
def create_benchmark_same_obf():
    ...
    for entry in read_jsonl_zip('../../data/splits/benchmark.jsonl.zip'):
    ...
    for overall_q_n, obfuscations in entries_by_overall_and_obf.items():
        ...
        for entry in chosen_entries:
            # Create new entry with a 'format' field added (index is left untouched
            # since downstream scripts assume index[0] == overall_question_n)
            new_entry = entry.copy()
            new_entry['format'] = format_mapping.get(overall_q_n, "Unknown")
    ...
    # Write output as a password-protected zip (matches the other files in data/splits)
    output_zip = '../../data/splits/benchmark_same_obf.jsonl.zip'
    write_jsonl_zip(output_zip, output_entries)

    print(f"\nCreated {output_zip} with {len(output_entries)} entries")
